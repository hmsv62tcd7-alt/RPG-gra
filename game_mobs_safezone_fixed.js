...
// ============================================
// VILLAGE NO-MOBS ZONE (HARD SAFE RECT)
// ============================================
const VILLAGE_NO_MOBS_RECT = { x: 120, y: 120, w: 780, h: 700 }; // x2=900, y2=820
const VILLAGE_BUFFER = 180; // Bufor żeby nie stali na krawędzi
...
const VILLAGE_NO_MOBS_RECT_PAD = expandRect(VILLAGE_NO_MOBS_RECT, VILLAGE_BUFFER);
const NEAR_VILLAGE_PAD = expandRect(VILLAGE_NO_MOBS_RECT, VILLAGE_BUFFER + 300); // Level gating zone
...
class Enemy {
    ...
    update(playerX, playerY) {
        // NO ENTRY: mob nie może wejść do wioski (HARD RECT)
        const r = VILLAGE_NO_MOBS_RECT_PAD;
        if (inRect(this.x, this.y, r)) {
            // Wypchnij na zewnątrz - najbliższa krawędź
            const left = Math.abs(this.x - r.x);
            const right = Math.abs((r.x + r.w) - this.x);
            const top = Math.abs(this.y - r.y);
            const bottom = Math.abs((r.y + r.h) - this.y);

            const m = Math.min(left, right, top, bottom);
            if (m === left) this.x = r.x - 5;
            else if (m === right) this.x = r.x + r.w + 5;
            else if (m === top) this.y = r.y - 5;
            else this.y = r.y + r.h + 5;
            
            this.isAggro = false;
            console.log('[NO_ENTRY] Mob', this.name, 'pushed out of village');
        }
...
class Game {
    constructor() {
        ...
        this.houseTexture.onload = () => {
            ...
            console.log('[Texture] House texture loaded');
        };
        ...
        this.stoneTexture.onload = () => {
            ...
            console.log('[Texture] Stone texture loaded');
        };
    ...
    // ============================================
    // VILLAGE SAFE ZONE HELPERS
    // ============================================

    computeVillageSafeRect(padding) {
        // Oblicz prostokąt pokrywający wszystkie budynki wioski + padding
        if (!this.villageBuildings || this.villageBuildings.length === 0) {
            // Fallback - hardcoded village rect
            return { x: 135, y: 95, w: 570, h: 590 };
        }
    ...
    projectOutsideRect(x, y, rect, extraDist = 20) {
        // Wypchnij punkt (x,y) na najbliższą krawędź rect + extraDist
        ...
        const pushDist = Math.max(rect.w, rect.h) / 2 + extraDist;
        return {
    ...
    generateEnemies() {
        ...
        const placeZone = (count, distMin, distMax, minSpacing, pool, difficulty, pairChance = 0.22) => {
            ...
            while (placed.length < count && attempts < count * 80) {
                ...
                // NO SPAWN: odrzuć jeśli w VILLAGE_NO_MOBS_RECT_PAD
                if (inRect(x, y, VILLAGE_NO_MOBS_RECT_PAD)) {
                    continue;
                }
                
                // LEVEL GATING: sprawdź czy blisko wioski
                const nearVillage = inRect(x, y, NEAR_VILLAGE_PAD);
                ...
                if (placed.length < count && rand() < pairChance) {
                    ...
                    // NO SPAWN dla grupy też - HARD RECT
                    if (inBounds(nx, ny) && !inRect(nx, ny, VILLAGE_NO_MOBS_RECT_PAD)) {
                        let ok = true;
                        for (const p of placed) {
                            const dx = p.x - nx;
                            const dy = p.y - ny;
            ...
            placed.slice(0, count).forEach((pos) => {
                // LEVEL GATING: przy wiosce tylko 1-4lv (wolf/boar/whelp/bear)
                const nearVillage = inRect(pos.x, pos.y, NEAR_VILLAGE_PAD);
                const lowPool = ['wolf', 'boar', 'whelp', 'bear'];
                const poolToUse = nearVillage ? lowPool : pool;
                
                const entry = poolToUse[Math.floor(rand() * poolToUse.length)];
                const cfg = mobCfg[entry] || mobCfg.boar;

                let baseName = cfg.names[Math.floor(rand() * cfg.names.length)];
                let level = cfg.level || 1;

                // ELITA: Golem Starożytny (rzadko, tylko w hard) - NIE przy wiosce
                if (entry === 'golem' && difficulty === 'hard' && !nearVillage && rand() < 0.10) {
    ...
    initMultiplayer() {
        ...
        try {
            ...
            // Fallback na users/player (jeśli playersOnline puste lub brak uprawnień)
...
window.addEventListener('load', () => {
    ...
    game = new Game();
    window.gameManager = game; // Dodane dla debug access
    initChat();
...
// ============================================
// DEBUG HELPERS - dostępne w konsoli
// ============================================

window.clearEnemiesTemplate = function() {
    console.log('[DEBUG] Clearing enemiesTemplate from Firebase...');
    if (!currentUser) {
        console.warn('[DEBUG] Not logged in - cannot clear Firebase data');
        return;
    }
    ...
    for (let mapId = 1; mapId <= 3; mapId++) {
        promises.push(
            database.ref('gameState/map' + mapId + '/enemiesTemplate').remove()
                .then(() => console.log('[DEBUG] Cleared map ' + mapId + ' enemies template'))
                .catch(err => console.error('[DEBUG] Error clearing map ' + mapId + ':', err))
        );
    }
    
    Promise.all(promises).then(() => {
        console.log('[DEBUG] All enemy templates cleared. Reloading page in 1s...');
